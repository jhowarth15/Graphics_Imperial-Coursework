...
void shpere_intersect(Sphere sph, Ray ray, inout Intersection intersect)
{
  ...
  float c = pow(length(ray.origin - sph.centre), 2.0) -
  pow(sph.radius, 2.0);
  ...
  if (d >= 0)
  {
	float m1 = (sqrt(disc) - b) / (2*a);
	float m2 = (-sqrt(disc) - b) / (2*a);
	float m;
...
vec3 computeShadow(in Intersection intersect)
{
  ...
  shadowRay = inter.point + shadowRay.dir*E;

  Intersection shadowInter;
  shadowInter.t = -1;
  Intersect(shadowRay, shadowInter);

  if (shadowInter.t != -1 && shadowInter.t < length(lightPosition -
  shadowRay.origin))
  {
	col += vec4(0,0,0,0);
  } else
  ...
  gl_LightSource[0].quadraticAttenuation*d*d);
	...
	vec4 id = attenuation * vec4(inter.colour, 1) *
  max(dot(inter.normal, lightVector), 0);
...
void main()
{
  ...
  while (currentDepth < rayTraceDepth)
  {
	...
	if (intersec.t > 0)
	   {
		col += pow(0.5, float(currentDepth)) * computeShadow(inter);
		intersec.colour = vec3(0,0,0);
		intersec.t = -1;
		depth++;
		...
		reflectedRay.dir = normalize(ray.dir - dot(2 *
		ray.dir, intersec.normal) * intersec.normal);
		reflectedRay.origin += E * reflectedRay.dir;
		ray = reflectedRay;
	   } else
	   {
		break;
